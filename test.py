...
from gtts import gTTS  # 引入跨平台 gTTS 免費引擎，確保手機端能出聲
...
def fetch_google_info(query):
    ...
    try:
        for url in search(query, num_results=2, lang="zh-TW"):
            if any(x in url for x in ['.pdf', 'youtube', 'youtu.be']): continue
...
if final_text and client:
    ...
    msgs.extend(st.session_state.messages[-3:]) 
    ...
    with st.chat_message("assistant", avatar="⚡"):
        ...
        # 🔊 【終極優化：gTTS MP3 + 1.35倍速 + 隱形行動端解鎖播放器】
        clean_txt = re.sub(r'[^\u4e00-\u9fa50-9，。！？]', '', full_res)
        if clean_txt:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as fp:
                tts = gTTS(text=clean_txt, lang='zh-tw', slow=False)
                tts.save(fp.name)
            ...
            js_tts_player = f"""
                <audio id="js-audio" style="display:none;" preload="auto">
                ...
                (function() {{
                    ...
                    if (audio) {{
                        audio.volume = 1.0;
                        audio.defaultPlaybackRate = 1.35; // ⚡ 1.35倍速噴射
                        audio.playbackRate = 1.35;
                        
                        var playPromise = audio.play();
                        if (playPromise !== undefined) {{
                            playPromise.catch(function(error) {{
                                // 如果手機系統阻擋了自動播放，註冊監聽器：使用者只要一摸螢幕就立刻播放
            ...
            """
            st.components.v1.html(js_tts_player, height=0)
